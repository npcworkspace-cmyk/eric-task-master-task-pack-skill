...
- **Reddit:** read `SKILL.md`, `references/data-contract.md`, and `references/runtime-adaptation.md`. Copy the Pack from `assets/reddit-comment-tree-pack/` to the task directory, prepare the documented input, and run `node scripts/verify-pack.mjs "ACTUAL_ENTRY"` from the installed Skill directory. Submit that entry through Task Master.
- **TikTok:** from the installed `tiktok-seed-discovery` directory, run `node scripts/doctor.mjs`. Follow `scripts/runtime/CONTRACT.md`: intake → reference collection → reference analysis and Agent review → search batches → centralized screening. Existing reviewed seeds enter the expansion Skill. Do not run the offline `process.mjs` as a browser task.
